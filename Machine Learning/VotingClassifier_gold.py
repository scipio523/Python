...
gdx = pd.read_csv('data/GDX.csv', index_col='Date')[-sample_size:]

# Define Features - past n day returns 
df = pd.DataFrame(index=data.index)
gld1 = data['Adj Close'].pct_change()
gdx1 = gdx['Adj Close'].pct_change()
df['Spread'] = gdx1 - gld1
df.dropna(inplace=True)  # Clear NaNs

# Define Target - future 1 day return
df['FutureReturn'] = gld1.shift(-1)  # we are predicting future day's return so shift 1 day ahead
